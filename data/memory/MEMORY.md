...
[tip] SKILL_PRE_ROUTER=1 opts into per-turn LLM skill router (default OFF). Default progressive-disclosure: catalog in system prompt (stable, cached), active bodies on last user message. test_skill_cache_stability.py pins the "system prompt byte-identical across activation" invariant.
...
[correction] Skills do NOT auto-activate under default config (SKILL_PRE_ROUTER=0) — the catalog is advertised in the system prompt and YOU must self-activate via `manage_skills(action='activate', name=...)` before executing when a user request matches a skill's TRIGGER clauses. Skipping activation because "I already know how" defeats the skill's checklists.
