...
[gotcha] Any tool with a long-running inner deadline MUST have its `timeout_hint` in `jyagent/tools/__init__.py` ≥ the inner cap, else the outer tool-invocation wrapper (`runtime/loop/tool_invocation.py:241-244`, `timeout = max(default, hint)`) silently undercuts it and returns a generic mutating-tool-timeout instead of the tool's own envelope. dispatch_agent is at 3600 (schema cap) as of commit 6f53808.
[gotcha] SearxNG container (`infra/searxng/`) routes outbound via host SOCKS5 proxy `socks5h://host.docker.internal:1080` (user's `ssh -D 1080`) to bypass GFW. If web_search returns 0 results with all engines timing out, first check the proxy is up: `lsof -nP -iTCP:1080 -sTCP:LISTEN`.
