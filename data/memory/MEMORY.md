...
[tip] Session persistence: append-only JSONL log at data/sessions/events/<sid>.jsonl is source of truth; latest.json + archives are view caches. Compaction emits `kind:compaction` event with drop_count+replacement_messages before swap. checkpoint flushes log first then snapshot. load_session auto-replays via replay_from_events on log-ahead.
