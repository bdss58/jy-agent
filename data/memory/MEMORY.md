...
[gotcha] Skill LLM router (jyagent/skills.py::_route_llm) was silently broken until 2025-11-21 fix: complete_text() in runtime/core.py unconditionally injected adaptive thinking via get_reasoning_config_for_provider, which validate_anthropic_reasoning rejects for any Anthropic model < Claude 4.6 (Haiku, Sonnet 4.5, Opus 4.5). The router's blanket `except Exception: return None` swallowed the ValueError every call → keyword fallback silently took over. Fixed by (1) adding reasoning= kwarg with _UNSET sentinel to complete_text, (2) router now passes reasoning=None (cheap routing call, no thinking needed), (3) except now logs dim warning instead of silent swallow. Regression tests in tests/test_skill_router.py (11 tests).
[workflow] GFW fallback proxy workflow: when a network op fails or times out against likely-blocked hosts (ghcr.io, raw.githubusercontent.com, huggingface.co, pypi/github LFS, docker.io pulls, google APIs, etc.), don't give up — route through the user's SSH SOCKS tunnel. Steps: (1) check listener: `lsof -iTCP:1080 -sTCP:LISTEN -nP`; (2) if missing, start it with `run_background` (NOT run_shell, it's long-lived): `ssh -NL 1080:localhost:1080 thost` — host alias is `thost` in user's ~/.ssh/config; (3) retry via SOCKS5: curl `--socks5-hostname 127.0.0.1:1080`, git `-c http.proxy=socks5h://127.0.0.1:1080`, env `ALL_PROXY=socks5h://127.0.0.1:1080 HTTPS_PROXY=socks5h://127.0.0.1:1080`, pip `--proxy socks5h://127.0.0.1:1080`. Always try direct first (often works), proxy only on failure. Use `socks5h://` (remote DNS) not `socks5://` to avoid DNS leaks/poisoning.
