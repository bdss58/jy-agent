"""Filesystem tools: read_file, write_file, list_directory, edit_file, glob/grep helpers.

Shared helpers (resolve_path, should_skip_dir, is_binary_ext, _track_file,
_format_size) live here too — search.py imports them from this module.

History: extracted from the former monolithic ``tools/core.py`` (2026-05).
The shell tool (``run_shell``) and its bounded-stream/spill machinery live
in ``tools/shell.py``; long-running background jobs live in
``tools/background.py``.
"""
import os
import difflib
