...
from .types import AssistantMessage, Message, ModelSpec, TextBlock, ThinkingBlock, ToolCallBlock, ToolResultMessage
...
def _thinking_to_text_block(block: ThinkingBlock) -> TextBlock | None:
...
def _inject_missing_tool_results(messages: list[Message]) -> list[Message]:
    out: list[Message] = []
...
def transform_messages_for_target(messages: list[Message], target: ModelSpec) -> list[Message]:
    ...
    for message in messages:
        ...
        for raw_block in assistant.get("content", []):
            ...
            if block_type == "thinking":
                thinking_block = cast(ThinkingBlock, raw_block)
                if same_model:
                    new_blocks.append(thinking_block)
                else:
                    if thinking_block.get("redacted") or (
                        thinking_block.get("encrypted_content") and not thinking_block.get("thinking", "").strip()
                    ):
                        continue
                    text_block = _thinking_to_text_block(thinking_block)
                    if text_block:
    ...
    return _inject_missing_tool_results(transformed)
